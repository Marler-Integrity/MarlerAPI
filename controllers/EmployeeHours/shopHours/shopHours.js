...
exports.updateSubmissions = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (!entries) return next(new ErrorResponse(`No Entries Present in Request`, 400))
        ...
        await t.rollback();
        console.log(error);
        return next(new ErrorResponse(`Server Error - updateSubmissions - ${error.message}`, 500));
    }
});
...
exports.deleteEntry = asyncHandler(async(req, res, next) => {
    try {
        ...
        console.log(error);
        return next(new ErrorResponse(`Server Error - deleteEntry - ${error.message}`));
    }
});
