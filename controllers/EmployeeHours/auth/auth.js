...
exports.userLogin = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!user) {
      return next(
        new ErrorResponse(
          `Cannot Find Your Email Address - Try Again or Register for an Account`,
          404
        )
      );
    }
    if (!user.IsActive)
      return next(new ErrorResponse(`Not Authorized - Inactive User`, 401));
...
exports.userRegister = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!Email || !Password || !FirstName || !LastName)
      return next(
        new ErrorResponse(
          `Please include Email, Password, FirstName and LastName in Request`,
          400
        )
      );
    ...
    if (!azureUserData)
      return next(
        new ErrorResponse(
          `Must use Marler Integrity Account - Cannot Find User in MS365`,
          400
        )
      );
    ...
    return next(
      new ErrorResponse(
        `Error Creating User - Register - ${error.message}`,
        500
      )
    );
  }
...
exports.fieldUserRegister = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!Email || !Password)
      return next(
        new ErrorResponse(
          `Please Provide an Email and Password in Request`,
          400
        )
      );
    ...
    try {
      ...
      if (user)
        return next(new ErrorResponse(`You already have an account`, 400));
    } catch (error) {
      console.log(error);
    ...
    if (!person)
      return next(
        new ErrorResponse(`Could Not Find Your Name in Our System`, 404)
      );
    ...
    console.log(error);
    return next(
      new ErrorResponse(`Server Error - fieldUserRegister - ${error.message}`)
    );
  }
});
...
exports.verifyEmail = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!user)
      return next(
        new ErrorResponse(`Invalid Verification Token - Try Again`, 400)
      );
    ...
    console.log(error);
    return next(
      new ErrorResponse(`Server Error - verifyEmail - ${error.message}`)
    );
  }
});
 ...
 * @route POST /api/v1/employeehours/auth/changepassword
...
exports.changePassword = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!oldPassword || !newPassword)
      return next(
        new ErrorResponse(400, `Please send old and new password in request`)
      );
    ...
    console.log(error);
    return next(new ErrorResponse(500, `Server Error - changePassword`, error));
  }
});
